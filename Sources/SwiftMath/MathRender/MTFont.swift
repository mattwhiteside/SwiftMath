...
public class MTFont {
  ...
  var rawMathTable: Dictionary<String, Any>?
  ...
  convenience init(fontWithName name: String, size:CGFloat) throws {
    self.init()
    ...
    self.rawMathTable = NSDictionary(contentsOf: mathTablePlist!) as? Dictionary<String, Any>
  ...
  public func copy(withSize size: CGFloat) throws -> MTFont {
    ...
    try newFont.mathTable = MTFontMathTable(withFont: newFont, mathTable: newFont.rawMathTable!)
    return newFont
  }
